...
const connectionOptions = {
    ...
    mobile: MethodMobile,
    browser: opcion === '1' ? ['𝐒𝐎𝐅𝐈 𝐁𝐎𝐓 ', 'Safari', '2.0.0'] : methodCodeQR ? ['𝐒𝐎𝐅𝐈 𝐁𝐎𝐓 ', 'Safari', '2.0.0'] : ['Ubuntu', 'Chrome', '20.0.04'],
    auth: {
        creds: state.creds,
...
let prekey = []
let directorio = readdirSync("./SofiSession")
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-') /*|| file.startsWith('session-') || file.startsWith('sender-') || file.startsWith('app-') */
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./SofiSession/${files}`)
})
} 
...
const directories = ['./SofiSession/', './jadibts/']
...
if (reason == 405) {
await fs.unlinkSync("./SofiSession/" + "creds.json")
